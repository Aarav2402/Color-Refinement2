...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        graph1 = self
        graph2 = other
        graph = Graph(False, len(graph1) + len(graph2), True)

        for i in graph1.edges:
            for j in graph.vertices:
                if j.label == i.tail.label:
                    vertex1 = j
                if j.label == i.head.label:
                    vertex2 = j
            graph.add_edge(Edge(vertex1,vertex2))

        for i in graph2.edges:
            for j in graph.vertices:
                if j.label == i.tail.label + len(graph1):
                    vertex1 = j
                if j.label == i.head.label + len(graph1):
                    vertex2 = j
            graph.add_edge(Edge(vertex1,vertex2))
        return graph
        pass
